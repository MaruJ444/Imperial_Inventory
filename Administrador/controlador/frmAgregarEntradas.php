                        ...
                        <form id="form1" name="form1" method="post" action="validarAgregarEntradas.php">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input name="producto" id="producto" style="width: 100%" class="form-control" pattern="[A-Za-z ]+" title="Solo se permiten letras." required />
...
<script>
    ...
    window.onload = function() {
        ...
        if (!<?php echo isset($_POST['button']) ? 'true' : 'false'; ?>) {
            window.onbeforeunload = function() {
                return "¿Está seguro que desea salir de la vista de agregar pedido?";
            };
        }
    ...
    document.getElementById('form1').addEventListener('submit', function(event) {
        ...
        var cantidadInput = parseInt(document.getElementById('cantidadentrada').value);
        var fechaInput = document.getElementById('fecha_entrada').value;
        var productos = document.querySelectorAll('.producto');
        var existeProducto = false;

        productos.forEach(function(item) {
            if (item.innerText.trim() === productoInput.trim()) {
                existeProducto = true;
                var cantidadActual = parseInt(item.nextElementSibling.innerText.trim());
                var fechaActual = item.nextElementSibling.nextElementSibling.innerText.trim();
                item.nextElementSibling.innerText = cantidadActual + cantidadInput;
                item.nextElementSibling.nextElementSibling.innerText = fechaInput;
            }
        });
